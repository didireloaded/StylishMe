...
type Order = { id: string; date: string; status: string; total: number; fulfilment: FulfilmentMethod; items: CartLine[] };
...
export default function StylishMeApp({
  ...
  const [wishlist, setWishlist] = useState<string[]>(["p2", "p4", "p7", "p9", "p11", "p14"]);
  ...
  else if (view === "tracking") content = <>{header(isCollectionOrder ? "Collection details" : "Order tracking", "orders")}{isCollectionOrder ? <><section className="tracking-head collection-head"><small>{selectedOrder?.id}</small><h1>Collection status</h1><h2>{selectedOrder?.status ?? "Preparing for collection"}</h2><p>We will notify you as soon as the store marks your order ready.</p></section><section className="collection-card"><small>COLLECTION STORE</small><h2>StylishMe · Independence Avenue</h2><strong>12 Independence Avenue</strong><p>Windhoek, Khomas</p><div><span>Mon–Sat</span><strong>09:00–18:00</strong></div><div><span>Collection code</span><strong>SM-4826</strong></div><p>Bring your collection code and a photo ID. No courier tracking is needed for this order.</p></section><ol className="timeline">{["Order confirmed", "Store preparing order", "Ready to collect", "Collected"].map((item, i) => <li className={i < (selectedOrder?.status === "Ready to collect" ? 3 : 2) ? "done" : ""} key={item}><i /><div><strong>{item}</strong><small>{i < 2 ? ["21 Jul · 10:12", "21 Jul · 10:46"][i] : "Pending"}</small></div></li>)}</ol></> : <><section className="tracking-head"><small>{selectedOrder?.id ?? "SM-2026-1048"}</small><h1>Delivery tracking</h1><h2>{selectedOrder?.status ?? "In transit"}</h2><p>Estimated delivery 18–20 July</p></section><ol className="timeline">{["Order confirmed", "Store preparing order", "Collected by courier", "In transit", "Out for delivery", "Delivered"].map((item, i) => <li className={i < 4 ? "done" : ""} key={item}><i /> <div><strong>{item}</strong><small>{i < 4 ? ["15 Jul · 09:42", "15 Jul · 12:18", "16 Jul · 08:06", "16 Jul · 14:20"][i] : "Pending"}</small></div></li>)}</ol></>}<button className="outline-button full" onClick={() => navigate("support")}>Contact support</button></>;
  ...
  else content = <>{header("Settings", "profile")}<section className="settings-card"><h2>Fit Passport</h2>{[["Normal clothing size", "size"], ["Shoe size", "shoe"], ["Preferred fit", "fit"]].map(([label, key]) => <label key={key}><span>{label}</span><input value={profile[key as keyof typeof profile]} onChange={e => setProfile(p => ({ ...p, [key]: e.target.value }))} /></label>)}<p>Recommendations are suggestions, not a fit guarantee.</p></section><section className="settings-card"><label className="switch-row"><span><strong>Data-light mode</strong><small>Reduce imagery and motion</small></span><input type="checkbox" checked={dataLight} onChange={e => setDataLight(e.target.checked)} /></label><label className="switch-row"><span><strong>Order notifications</strong><small>Delivery and collection updates</small></span><input type="checkbox" defaultChecked /></label></section></>;
