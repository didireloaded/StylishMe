...
import { cancelAccountDeletion, checkAccountDeletionEligibility, processDueAccountDeletions, scheduleAccountDeletion } from "../app/account-deletion.ts";
...
function database() {
  ...
  db.database.prepare("INSERT INTO seller_state VALUES ('seller-1',NULL,0,'Didi Studio','{}','2026-07-01')").run();
  db.database.prepare("INSERT INTO catalog_products VALUES ('product-1','seller-1','archived','2026-07-01')").run();
  db.database.prepare("INSERT INTO commerce_orders VALUES ('order-1','didi@example.com','completed')").run();
...
test("account deletion can be scheduled and cancelled during the grace period", async () => {
  ...
  assert.equal(scheduled.scheduledFor, "2026-08-05T10:00:00.000Z");
...
test("account deletion is blocked by an active seller store and active order", async () => {
  const db = database();
  db.database.prepare("UPDATE seller_state SET owner_email = 'didi@example.com', approved = 1").run();
  db.database.prepare("UPDATE commerce_orders SET status = 'confirmed'").run();
  const eligibility = await checkAccountDeletionEligibility(db, "didi@example.com");
  assert.deepEqual(eligibility.blockers.map(item => item.code), ["active_seller_store", "active_order"]);
  await assert.rejects(() => scheduleAccountDeletion(db, "didi@example.com"), /resolve account obligations/i);
  db.close();
});
