...
import StoreClosureControl from "./StoreClosureControl";

type View = "today" | "collection" | "add" | "orders" | "inventory" | "payouts" | "notifications" | "settings" | "store" | "more" | "collections" | "customers" | "questions" | "analytics";
type Status = "Live" | "Changes requested" | "Draft";
type SellerProduct = {
  id: string; name: string; description: string; category: string; collection: string;
  price: number; salePrice?: number; badge?: "New Arrival" | "Limited Drop" | ""; material: string; fit: string; colours: string[];
...
const products: SellerProduct[] = [
  { id: "sp1", name: "Ondelela Evening Dress", description: "A fluid ceremony dress cut and finished in Windhoek.", category: "Women", collection: "Modern Ceremony", price: 2450, badge: "Limited Drop", material: "Viscose satin", fit: "Regular", colours: ["Coral", "Black"], variants: [{ size: "S", colour: "Coral", quantity: 2 }, { size: "M", colour: "Coral", quantity: 4 }, { size: "L", colour: "Black", quantity: 1 }], images: [images[0]], delivery: ["Nationwide delivery", "Store collection"], returns: "14 days", madeToOrder: true, status: "Live" },
  { id: "sp2", name: "Walvis Linen Co-ord", description: "Breathable tailoring designed for relaxed weekends along the coast.", category: "Women", collection: "Coastline Weekend", price: 1399, salePrice: 1199, badge: "New Arrival", material: "Linen blend", fit: "Relaxed", colours: ["Sand"], variants: [{ size: "S", colour: "Sand", quantity: 1 }, { size: "M", colour: "Sand", quantity: 2 }], images: [images[1]], delivery: ["Nationwide delivery"], returns: "14 days", madeToOrder: false, status: "Live" },
  { id: "sp3", name: "Lilac Ceremony Set", description: "A polished two-piece set for celebrations.", category: "Women", collection: "Modern Ceremony", price: 2190, badge: "", material: "Crepe", fit: "Tailored", colours: ["Lilac"], variants: [{ size: "M", colour: "Lilac", quantity: 0 }], images: [images[2]], delivery: ["Store collection"], returns: "Made-to-order pieces are final sale", madeToOrder: true, status: "Changes requested" },
];
...
const blank = (): SellerProduct => ({ id: "", name: "", description: "", category: "Women", collection: "", price: 0, badge: "", material: "", fit: "Regular", colours: ["Coral"], variants: [{ size: "M", colour: "Coral", quantity: 1 }], images: [], delivery: ["Nationwide delivery"], returns: "14 days", madeToOrder: false, status: "Draft" });
...
export default function SellerApp({
  ...
  else if (view === "notifications") content = <>{header("Notifications")}<section className="page-intro"><small>STORE UPDATES</small><h1>Notifications</h1><p>Order, stock, product and payout updates will land here when your store is active.</p></section><section className="seller-empty large"><strong>You are all caught up</strong><small>No seller notifications need attention right now.</small></section></>;
  else if (view === "settings") content = <>{header("Settings")}<section className="page-intro"><small>STORE CARE</small><h1>Settings</h1><p>Keep the small things that shape customer trust in one calm place.</p></section><section className="settings-list"><article><strong>Low-stock alert</strong><small>Highlight pieces with fewer than 3 units.</small><span>On</span></article><article><strong>Store visibility</strong><small>Your storefront can be shared with customers.</small><span>{state.store.approved ? "Open" : "Draft"}</span></article><article><strong>Account security</strong><small>Managed securely through your StylishMe account.</small><span>Protected</span></article></section>{!demoMode && <StoreClosureControl storeName={state.store.name} />}</>;
  else if (view === "collections") content = <>{header("Collections")}<section className="page-intro"><small>MERCHANDISING</small><h1>Collections</h1><p>Group pieces into small edits customers can understand quickly.</p></section><section className="seller-story-list">{["Modern Ceremony", "Coastline Weekend", "New arrivals"].map((label) => <article key={label}><span>{label === "New arrivals" ? "New" : label.split(" ").map(word => word[0]).join("")}</span><div><strong>{label}</strong><small>{state.products.filter(product => product.collection === label || (label === "New arrivals" && product.status === "Live")).length} pieces</small></div><button onClick={() => go("collection")}>View</button></article>)}</section></>;
  else if (view === "customers") content = <>{header("Customers")}<section className="page-intro"><small>RELATIONSHIPS</small><h1>Customers</h1><p>Customer profiles will appear after real orders, saves and messages.</p></section><section className="seller-empty large"><strong>No customer activity yet</strong><small>The demo keeps private customer data out, but this is where returning shoppers and order history will feel organized.</small></section></>;
  else if (view === "questions") content = <>{header("Reviews")}<section className="page-intro"><small>CONVERSATIONS</small><h1>Reviews & questions</h1><p>Keep product questions and customer feedback close to the pieces they are about.</p></section><section className="seller-story-list"><article><span>Q</span><div><strong>No open questions</strong><small>Questions from shoppers will appear here before purchase.</small></div><button onClick={() => go("collection")}>Products</button></article><article><span>★</span><div><strong>No reviews yet</strong><small>Verified order reviews will show after fulfilment.</small></div><button onClick={() => go("orders")}>Orders</button></article></section></>;
  else if (view === "analytics") content = <>{header("Insights")}<section className="page-intro"><small>STORE INSIGHTS</small><h1>Analytics</h1><p>A simple view of what customers are noticing, saving and buying.</p></section><section className="insight-board"><article><small>Most ready</small><strong>{live[0]?.name ?? "Add your first piece"}</strong><span>{live[0] ? `${totalStock(live[0].variants)} units available` : "Start your collection"}</span></article><article><small>Watch</small><strong>{low[0]?.name ?? "Stock is healthy"}</strong><span>{low[0] ? "Low-stock piece" : "No urgent stock signals"}</span></article><article><small>Next</small><strong>Share your store</strong><span>Send customers directly to your collection.</span></article></section></>;
  else if (view === "more") content = <>{header("More")}<section className="page-intro"><small>YOUR STORE</small><h1>Everything else</h1><p>Helpful seller spaces without making the app feel heavy.</p></section><section className="seller-more-grid"><button onClick={() => go("collections")}><strong>Collections</strong><small>Build edits and seasonal stories</small></button><button onClick={() => go("customers")}><strong>Customers</strong><small>Private shopper activity</small></button><button onClick={() => go("questions")}><strong>Reviews & Questions</strong><small>Feedback and product questions</small></button><button onClick={() => go("analytics")}><strong>Analytics</strong><small>Simple store insights</small></button><button onClick={() => go("payouts")}><strong>Payouts</strong><small>Verified finance records</small></button><button onClick={() => go("settings")}><strong>Settings</strong><small>Store preferences</small></button></section></>;
  else content = <>
