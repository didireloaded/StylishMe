...
import { catalogueProductUrl, matchesProductSlug, matchesStoreSlug } from "./unified-domain";

type LiveSellerProduct = {
  id: string; name: string; description: string; price: number; salePrice?: number;
  images: string[]; collection?: string; category?: string; productSlug?: string;
...
export default function StorefrontView({
  ...
  const legacyProduct = useMemo(() => typeof window === "undefined" ? "" : new URLSearchParams(window.location.search).get("product") ?? "", []);
  const selectedSlug = linkedProduct || legacyProduct;
  ...
  const [selected, setSelected] = useState<LiveSellerProduct | Product | null>(
    () => seeded.find((product) => matchesProductSlug(product.name, selectedSlug) || product.id === selectedSlug) ?? null,
  );
  ...
  useEffect(() => {
    fetch(`/api/catalog?store=${encodeURIComponent(storeSlug)}`)
      ...
      .then((data) => {
        ...
        setRemote(products);
        const match = products.find((product) => product.productSlug === selectedSlug || matchesProductSlug(product.name, selectedSlug) || product.id === selectedSlug);
        if (match) setSelected(match);
      })
      .catch(() => setRemote([]));
  }, [selectedSlug, storeSlug]);
  ...
  const exit = () => onExit ? onExit() : window.location.assign("/");
  const openProduct = (product: LiveSellerProduct | Product) => {
    setSelected(product);
    const path = catalogueProductUrl(storeSlug, isLiveSellerProduct(product) ? product.productSlug || product.name : product.name);
    window.history.replaceState({}, "", path);
  };
  const closeProduct = () => {
    setSelected(null);
    window.history.replaceState({}, "", `/stores/${encodeURIComponent(storeSlug)}`);
  };

  return <main className="storefront-stage">
    <div className="storefront-shell">
      <header><button onClick={exit}>← Explore StylishMe</button><strong>STYLISHME</strong><span>♡</span></header>
      ...
      <section className="storefront-grid" aria-label={`Products from ${storeName || "this store"}`}>
        {pieces.map((product) => <article key={product.id}>
          <button className="storefront-image" onClick={() => openProduct(product)}>
          ...
          <button onClick={() => openProduct(product)}><strong>{product.name}</strong></button>
      ...
      <button className="storefront-exit" onClick={exit}>Explore StylishMe</button>
      {selected && <div className="storefront-sheet" role="dialog" aria-modal="true" aria-label={selected.name} onClick={closeProduct}>
        <section onClick={(event) => event.stopPropagation()}>
          <button aria-label="Close product" onClick={closeProduct}>×</button>
          ...
          <button className="storefront-shop-button" onClick={exit}>Continue in StylishMe to choose options</button>
        </section>
      </div>}
    </div>
  </main>;
