...
export default function StylishMeApp({
  ...
  else content = <>{header("Settings", "profile")}<section className="settings-card"><h2>Fit Passport</h2>{[["Normal clothing size", "size"], ["Shoe size", "shoe"], ["Preferred fit", "fit"]].map(([label, key]) => <label key={key}><span>{label}</span><input value={profile[key as keyof typeof profile]} onChange={e => setProfile(p => ({ ...p, [key]: e.target.value }))} /></label>)}<p>Recommendations are suggestions, not a fit guarantee.</p></section><AccountDeletionControl /><section className="settings-card"><label className="switch-row"><span><strong>Data-light mode</strong><small>Reduce imagery and motion</small></span><input type="checkbox" checked={dataLight} onChange={e => setDataLight(e.target.checked)} /></label><label className="switch-row"><span><strong>Order notifications</strong><small>Delivery and collection updates</small></span><input type="checkbox" defaultChecked /></label></section></>;
