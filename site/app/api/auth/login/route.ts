...
export async function POST(request: Request) {
  try {
    ...
    if (!await consumeAuthAttempt(request, email, 10)) return Response.json({ error: "Too many attempts. Try again in 15 minutes." }, { status: 429 });
    ...
    await clearAuthAttempts(request, email);
