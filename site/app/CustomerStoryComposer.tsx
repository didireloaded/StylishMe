...
export default function CustomerStoryComposer({
  ...
  return <section className="customer-story-overlay" role="dialog" aria-modal="true" aria-label="Share an outfit">
    <form className="customer-story-composer" onSubmit={submit}>
      ...
      <section><h2>Tag the purchased piece and store</h2><p>A store tag is required. Only delivered or collected StylishMe purchases can be tagged.</p><div className="customer-story-products">{eligibleProducts.map(product => <label key={product.id}><input type="checkbox" checked={selected.includes(product.id)} onChange={() => setSelected(current => current.includes(product.id) ? current.filter(id => id !== product.id) : [...current, product.id])} /><img src={product.image} alt="" /><span><strong>{product.name}</strong><small>From {product.designer}</small></span></label>)}</div></section>
