globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/AdminDashboard-vTJZZu4N.js","assets/layout-segment-context-BTwrApg1.js"];
...
function getDraftSecret() {
	return "7082eb0a-f6f8-4418-bb2e-56b09bd2d641";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "6248f581-497c-446e-9a54-46815d20b9d6";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "6248f581-497c-446e-9a54-46815d20b9d6") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "6248f581-497c-446e-9a54-46815d20b9d6",
		rootBoundaryId
	});
}
/**
...
async function getChatGPTUser() {
	...
	if (encodedName && encoding === "percent-encoded-utf-8") try {
...
async function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AdminDashboard_default, { operatorName: (await getChatGPTUser())?.displayName ?? "StylishMe owner" });
}
...
async function GET() {
	if (!await getChatGPTUser()) return Response.json({ error: "Sign in to open StylishMe Admin" }, { status: 401 });
	const origin = process.env.STYLISHME_API_ORIGIN;
	const secret = process.env.STYLISHME_ADMIN_API_KEY;
	if (!origin || !secret) return Response.json({ error: "The live marketplace connection is not ready" }, { status: 503 });
	try {
...
function secure(response) {
	...
	headers.set("cache-control", response.headers.get("content-type")?.includes("text/html") ? "no-store, private" : headers.get("cache-control") ?? "no-store");
...
var worker_entry_default = { async fetch(request, env, ctx) {
	...
	return secure(await app_router_entry_default.fetch(request, env, ctx));
} };
//#endregion
